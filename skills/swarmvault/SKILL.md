...
version: "3.21.0"
metadata: '{"openclaw":{"requires":{"anyBins":["swarmvault","vault"]},"install":[{"id":"node","kind":"node","package":"@swarmvaultai/cli","bins":["swarmvault","vault"],"label":"Install Beehive CLI (npm)"}],"emoji":"🗃️","homepage":"https://www.swarmvault.ai/docs"}}'
---
...
- Set `SWARMVAULT_WORKSPACE_ID=<id>` once per shell, or pass `--workspace-id <id>` on Beehive behavior commands. Generated artifacts live under `<artifact-base>/<workspace_id>` when a workspace id is active.
...
- Use `swarmvault demo --no-serve` when the user wants the fastest zero-config walkthrough before pointing Beehive at their own sources.
...
- Read the active `swarmvault.schema.md` before compile or query work. It is the vault's operating contract and lives under `<artifact-base>/<workspace_id>/` when a workspace id is active.
- If `wiki/graph/report.md` exists under the active artifact root, use it before broad repo search.
- If `SWARMVAULT_OUT` is set, resolve generated artifacts from that output root. If `SWARMVAULT_WORKSPACE_ID` is also set, `raw/`, `wiki/`, `state/`, `agent/`, `inbox/`, and `swarmvault.schema.md` live under `<SWARMVAULT_OUT>/<workspace_id>/`; `swarmvault.config.json` stays in the project root.
...
- `swarmvault install --agent claude --mcp` also registers the Beehive MCP server in the project's `.mcp.json`; start that server with `SWARMVAULT_WORKSPACE_ID=<id>` or a `--workspace-id <id>` client arg, and include the same `workspace_id` in MCP tool calls. `swarmvault install --agent claude --hook --scope user` installs the skill, hook, and settings once under `~/.claude` for all repos (the hook no-ops in repos without a compiled graph report).

...
- Set `SWARMVAULT_WORKSPACE_ID=<id>` once per shell, or pass `--workspace-id <id>` on SwarmVault behavior commands. Generated artifacts live under `<artifact-base>/<workspace_id>` when a workspace id is active.
...
- `swarmvault install --agent claude --mcp` also registers the SwarmVault MCP server in the project's `.mcp.json`; start that server with `SWARMVAULT_WORKSPACE_ID=<id>` or a `--workspace-id <id>` client arg, and include the same `workspace_id` in MCP tool calls. `swarmvault install --agent claude --hook --scope user` installs the skill, hook, and settings once under `~/.claude` for all repos (the hook no-ops in repos without a compiled graph report).

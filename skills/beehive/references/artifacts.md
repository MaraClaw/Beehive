...
Paths below are relative to the active artifact root. By default that is the vault root or `BEEHIVE_OUT`; when `BEEHIVE_WORKSPACE_ID=<id>` is active, it is `<artifact-base>/<id>`. `beehive.config.json` stays at the project root.

## Canonical Inputs

- `beehive.schema.md` - vault instructions, naming rules, exclusions, freshness rules
...
- `wiki/outputs/chat-sessions/` - persisted chat transcripts from `beehive chat`
...
- `wiki/graph/share-card.svg` - 1200x630 visual graph share card generated by compile or `beehive graph share --svg`
- `wiki/graph/share-kit/` - portable share bundle with markdown, post text, SVG, HTML preview, and JSON metadata generated by compile or `beehive graph share --bundle`
...
- `state/chat-sessions/` - structured session state for resumable `beehive chat` conversations

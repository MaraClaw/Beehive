...
- **`state/retrieval/` local index** — the SQLite FTS5 shard and retrieval manifest stay fast up to the large tier. Above ~1 M indexed rows, plan on splitting the vault or moving the hybrid search layer to an external backend. Use `beehive retrieval status|doctor|rebuild` to inspect freshness and repair the local shard.
...
All knobs live in `beehive.config.json`:
...
Run `beehive benchmark` after a compile. The artifact lives at `state/benchmark.json` and ships with a per-source-class breakdown (since 0.11.0). The numbers you care about for scale planning:
...
1. Split by project — most vaults above 50,000 sources have a natural split (multiple repos, multiple research programs). Use separate `beehive init` roots and point each at a focused subset.

...
const memoryNoteSchema = z.object({ id: z.string().min(1), text: z.string().min(1), createdAt: z.string().min(1) });
const memoryTaskSchema = z.object({
  ...
  decisions: z.array(memoryNoteSchema).default([]),
...
type MemoryDirs = { stateDir: string; tasksStateDir: string; wikiDir: string; tasksWikiDir: string; indexPath: string };

export type MemoryTaskStoredPage = { task: AgentMemoryTask; page: GraphPage; content: string; contentHash: string };

export type MemoryTaskContextMetadata = {
  readonly id: string;
...
async function uniqueMemoryTaskPaths(
  ...
  const base = `${createdAt.replace(/[:.]/g, "-")}-${slugify(goal)}`;
  let id = base;
...
export async function ensureMemoryLedger(rootDir: string): Promise<{ changed: string[] }> {
  ...
  await ensureDir(dirs.tasksStateDir);
  await ensureDir(dirs.tasksWikiDir);
  const content = renderMemoryTaskIndex(await listMemoryTasks(rootDir), new Date().toISOString());
  if (await fileExists(dirs.indexPath)) {
    return { changed: [] };
  }
  await fs.writeFile(dirs.indexPath, content, "utf8");
  return { changed: [toPosix(path.relative(rootDir, dirs.indexPath))] };
}

async function refreshMemoryIndex(rootDir: string): Promise<void> {
  const dirs = memoryDirs(rootDir);
  await ensureDir(dirs.wikiDir);
  await fs.writeFile(dirs.indexPath, renderMemoryTaskIndex(await listMemoryTasks(rootDir), new Date().toISOString()), "utf8");
}

export async function persistMemoryTaskRecord(rootDir: string, task: AgentMemoryTask): Promise<AgentMemoryTaskResult> {
  ...
  return { task: normalized, artifactPath: normalized.artifactPath, markdownPath: normalized.markdownPath };
}

export async function createMemoryTaskDraft(
  rootDir: string,
  options: Pick<StartMemoryTaskOptions, "goal" | "target" | "agent">
...
export function applyMemoryTaskContextMetadata(task: AgentMemoryTask, metadata: MemoryTaskContextMetadata): AgentMemoryTask {
  return {
    ...task,
    contextPackIds: uniqueStrings([...task.contextPackIds, metadata.id]),
    sourceIds: uniqueStrings([...task.sourceIds, ...metadata.relatedSourceIds]),
    pageIds: uniqueStrings([...task.pageIds, ...metadata.relatedPageIds]),
    nodeIds: uniqueStrings([...task.nodeIds, ...metadata.relatedNodeIds])
  };
}
...
async function resolveMemoryTaskArtifactPath(rootDir: string, target: string): Promise<string | null> {
  ...
  if (isPathWithin(dirs.tasksStateDir, direct) && direct.endsWith(".json") && (await fileExists(direct))) return direct;
  const byId = path.resolve(dirs.tasksStateDir, `${target.replace(/\.json$/, "")}.json`);
  if (isPathWithin(dirs.tasksStateDir, byId) && (await fileExists(byId))) return byId;
...
export async function updateMemoryTaskRecord(
  rootDir: string,
  target: string,
  options: UpdateMemoryTaskOptions,
  contextMetadata?: MemoryTaskContextMetadata
): Promise<AgentMemoryTaskResult> {
  const task = await readMemoryTask(rootDir, target);
  if (!task) throw new Error(`Task not found: ${target}`);
  const updatedAt = new Date().toISOString();
  const nextStatus = options.status ? memoryTaskStatusSchema.parse(options.status) : task.status;
  const next = applyMemoryTaskContextMetadata(
    {
      ...task,
      status: nextStatus,
      updatedAt,
      ...
      notes: options.note
        ? [
            ...task.notes,
            { id: noteId("note", updatedAt, task.notes.length), text: normalizeWhitespace(options.note), createdAt: updatedAt }
          ]
        : task.notes,
    ...
    contextMetadata ?? { id: options.contextPackId ?? "", relatedSourceIds: [], relatedPageIds: [], relatedNodeIds: [] }
  );
  return await persistMemoryTaskRecord(rootDir, next);
}

export async function attachContextPackToMemoryTask(
  rootDir: string,
  target: string,
  metadata: MemoryTaskContextMetadata
): Promise<AgentMemoryTaskResult> {
  return await updateMemoryTaskRecord(rootDir, target, { contextPackId: metadata.id }, metadata);
}
...
export async function finishMemoryTask(rootDir: string, target: string, options: FinishMemoryTaskOptions): Promise<AgentMemoryTaskResult> {
  ...
  if (!outcome) throw new Error("Task outcome is required.");
  const task = await readMemoryTask(rootDir, target);
  if (!task) throw new Error(`Task not found: ${target}`);
  return await persistMemoryTaskRecord(rootDir, {
    ...task,
    status: "completed",
...
export function buildMemoryGraphElements(tasks: AgentMemoryTask[], pages: GraphPage[]): { nodes: GraphNode[]; edges: GraphEdge[] } {
  ...
  for (const task of tasks) {
    ...
    const touchedNodeIds = uniqueStrings(
      task.changedPaths.flatMap(
        (changedPath) =>
          pagesByPath.get(changedPath)?.nodeIds ??
          [...pagesByPath.values()].find((candidate) => candidate.path.endsWith(changedPath))?.nodeIds ??
          []
      )
    );

...
import type { SwarmVaultSettings } from "../settings/defaults";

const DEFAULT_PLUGIN_WORKSPACE_ID = "default";
const WORKSPACE_ID_PATTERN = /^[A-Za-z0-9][A-Za-z0-9_-]*$/;

export function resolvePluginWorkspaceId(settings: Pick<SwarmVaultSettings, "workspaceId">): string {
  ...
  if (!WORKSPACE_ID_PATTERN.test(configured)) {
    throw new Error(
      `Invalid SwarmVault workspace ID "${configured}". Use letters, numbers, underscores, and hyphens, starting with a letter or number.`
    );
  }
  return configured;
}

export function workspaceCliEnv(settings: Pick<SwarmVaultSettings, "workspaceId">): NodeJS.ProcessEnv {
  return {
    ...process.env,
...
export function workspaceArtifactRoot(workspaceRoot: string, settings: Pick<SwarmVaultSettings, "workspaceId">): string {

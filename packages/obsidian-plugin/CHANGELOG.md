...
- `BeehivePlugin` loads and unloads cleanly; status bar indicator; settings tab with CLI path and Verify CLI button.
...
- Release-sync check extended to pin plugin `package.json`, `manifest.json`, and `beehiveCliMinVersion` to the monorepo root version.

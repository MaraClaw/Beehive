...
1. **whisper.cpp binary** on `$PATH`:
   ...
   - Beehive searches for `whisper-cli`, `whisper-cpp`, or `whisper` in that order; override with `BEEHIVE_WHISPER_BINARY` or `localWhisper.binaryPath` in `beehive.config.json`.
2. **A Whisper ggml model** — `beehive provider setup --local-whisper` downloads `base.en` (~147 MB) into `~/.beehive/models/` by default. Pick a different tier with `--model {tiny.en,small.en,medium.en,large-v3}`.
...
beehive add sample.wav
beehive compile

# 5. Query the new page
beehive query "What did Dana cover in the quarterly review?"
```
...
- This example does **not** commit a binary fixture — generate one with the `say` / `ffmpeg` / `espeak` lines above, or point `beehive add` at any audio file on disk.
